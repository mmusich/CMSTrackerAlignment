...
#ifndef OfflineValidationTreeAna_h
#define OfflineValidationTreeAna_h
...
class OfflineValidationTreeAna {
public :
 ...
 //TkOffTreeVariables *TkOffTreeVariables;
   ...
   OfflineValidationTreeAna(TString filename="",TString theName="");
   virtual ~OfflineValidationTreeAna();
   ...
   virtual void     makeNicePlotStyleA(TH1F *hist);
   virtual void     makeNicePlotStyleB(TH1F *hist);
...
#ifdef OfflineValidationTreeAna_cxx
OfflineValidationTreeAna::OfflineValidationTreeAna(TString filename,TString theName) : fChain(0) 
{
  // if parameter tree is not specified (or zero), connect the file
  // used to generate this class and read the Tree.
  TTree *tree=0;
  if (tree == 0) {
    //TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("AlignmentValidation_validation_2012D_isoMuons_reprocessing_FT_R_53_V21.root");
    TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject(filename);
    if (!f || !f->IsOpen()) {
      // f = new TFile("AlignmentValidation_validation_2012D_isoMuons_reprocessing_FT_R_53_V21.root");
      f = new TFile(filename);
    }
    //TDirectory * dir = (TDirectory*)f->Get("AlignmentValidation_validation_2012D_isoMuons_reprocessing_FT_R_53_V21.root:/TrackerOfflineValidationStandalone");
...
OfflineValidationTreeAna::~OfflineValidationTreeAna()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t OfflineValidationTreeAna::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t OfflineValidationTreeAna::LoadTree(Long64_t entry)
{
...
void OfflineValidationTreeAna::Init(TTree *tree)
{
...
Bool_t OfflineValidationTreeAna::Notify()
{
...
void OfflineValidationTreeAna::Show(Long64_t entry)
{
...
Int_t OfflineValidationTreeAna::Cut(Long64_t entry)
{
...
void OfflineValidationTreeAna::makeNicePlotStyle(TH1F *hist){
  hist->SetLineWidth(2);
...
void OfflineValidationTreeAna::makeNice2DPlotStyle(TH2F *hist){
  hist->SetLineWidth(3);
  hist->GetXaxis()->CenterTitle(true);
  hist->GetYaxis()->CenterTitle(true);
  hist->GetXaxis()->SetTitleFont(42); 
...
void OfflineValidationTreeAna::makeNiceProfileStyleAndColor(TProfile *hist,int color){
...
void OfflineValidationTreeAna::makeNicePlotStyleAndColor(TH1F *hist,int color){
...
void OfflineValidationTreeAna::makeNicePlotStyleA(TH1F *hist){
  hist->SetLineColor(1);
  hist->SetLineStyle(9);
  hist->SetLineWidth(2);
  hist->GetXaxis()->CenterTitle(true);
...
void OfflineValidationTreeAna::makeNicePlotStyleB(TH1F *hist){
  hist->SetLineColor(2);
  hist->SetLineStyle(9);
  hist->SetLineWidth(2);
  hist->GetXaxis()->CenterTitle(true);
...
void OfflineValidationTreeAna::setStyle(TString palettename){
...
TPaveText* OfflineValidationTreeAna::addNiceTLegend(TObjArray *histos){
...
void OfflineValidationTreeAna::makeNiceCanv(TCanvas *c){
...
#endif // #ifdef OfflineValidationTreeAna_cxx
