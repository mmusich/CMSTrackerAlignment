#define OfflineValidationTreeAnalysis_v2_cxx
#include "OfflineValidationTreeAnalysis_v2.h"
...
void OfflineValidationTreeAnalysis_v2::Loop()
{
  //   In a ROOT session, you can do:
  //      Root > .L OfflineValidationTreeAnalysis_v2.C
  //      Root > OfflineValidationTreeAnalysis_v2 t
  ...
  static int numberOfLayers[8] = {3,3,2,2,4,3,6,9};
  static int nSubDets = 8;

  TCanvas *cProfRMSVsEta[8][9];
  TCanvas *cProfRMSVsPhi[8][9];
  TCanvas *cProfRMSVsZ[8][9];
  
  TCanvas *cPullRMSVsEta[8][9];
  TCanvas *cPullRMSVsPhi[8][9];
  TCanvas *cPullRMSVsZ[8][9];

  for(int iS = 0; iS<nSubDets; iS++){

    TString subDetName;
    switch (iS) {
    case 0: subDetName = "BPIX-x"; break;
    case 1: subDetName = "BPIX-y"; break;
    case 2: subDetName = "FPIX-x"; break;
    case 3: subDetName = "FPIX-y"; break;
    case 4: subDetName = "TIB"; break;
    case 5: subDetName = "TID"; break;
    case 6: subDetName = "TOB"; break;
    case 7: subDetName = "TEC"; break;
    }
    
    for(int iL = 0; iL<numberOfLayers[iS]; iL++){

      //std::cout<<"SubDet: "<<subDetName<<", Layer:"<<iL<<std::endl;
      //std::cout<<"text:" << Form("%s_cProf%sLayer%iEta",c_fChainName,subDetName.Data(),iL) <<std::endl;

      cProfRMSVsEta[iS][iL] = new TCanvas(Form("%s_cProf%sLayer%iEta",c_fChainName,subDetName.Data(),iL),Form("%s L-%i median vs #eta",subDetName.Data(),iL),700,700);
      cProfRMSVsPhi[iS][iL] = new TCanvas(Form("%s_cProf%sLayer%iPhi",c_fChainName,subDetName.Data(),iL),Form("%s L-%i median vs #phi",subDetName.Data(),iL),700,700);
      cProfRMSVsZ[iS][iL]   = new TCanvas(Form("%s_cProf%sLayer%iZ"  ,c_fChainName,subDetName.Data(),iL),Form("%s L-%i median vs z"   ,subDetName.Data(),iL),700,700);
      cPullRMSVsEta[iS][iL] = new TCanvas(Form("%s_cPull%sLayer%iEta",c_fChainName,subDetName.Data(),iL),Form("%s L-%i pull vs #eta",subDetName.Data(),iL),700,700);
      cPullRMSVsPhi[iS][iL] = new TCanvas(Form("%s_cPull%sLayer%iPhi",c_fChainName,subDetName.Data(),iL),Form("%s L-%i pull vs #phi",subDetName.Data(),iL),700,700);
      cPullRMSVsZ[iS][iL]   = new TCanvas(Form("%s_cPull%sLayer%iZ"  ,c_fChainName,subDetName.Data(),iL),Form("%s L-%i pull vs z"   ,subDetName.Data(),iL),700,700);
    }
  }
  ...
  Double_t theZMax[8] = {25.,25.,52.,52.,62.,112.,102.,276.}; // z boundaries of subdetector in cm
  std::pair<Double_t,Double_t> theRadii[8];
  
  theRadii[0] = std::make_pair(0.,12.);
  theRadii[1] = std::make_pair(0.,12.);
  theRadii[2] = std::make_pair(6.,14.);
  theRadii[3] = std::make_pair(6.,14.);

  theRadii[4] = std::make_pair(23.,52.);
  theRadii[5] = std::make_pair(28.,45.);
  theRadii[6] = std::make_pair(58.,111.);
  theRadii[7] = std::make_pair(25.,100.); 
  ...
  */
  ...
  for (UInt_t i=0;i<8;i++){
    ...
    profMedianPhi[dets[i]] = new TProfile(Form("prof_%s_VsPhi",dets[i].Data()),
					  ...
					  36,-TMath::Pi(),TMath::Pi(),mymap[dets[i]].first,mymap[dets[i]].second,"s");
    ...
    profMedianZ[dets[i]]   = new TProfile(Form("prof_%s_VsZ",dets[i].Data()),
					  ...
					  10,-theZMax[i],theZMax[i],mymap[dets[i]].first,mymap[dets[i]].second,"s");
    // profPull
    ...
    profPullPhi[dets[i]]   = new TProfile(Form("profNormRes_%s_VsPhi",dets[i].Data()),
					  ...
					  36,-TMath::Pi(),TMath::Pi(),0.7,1.2,"i");
    ...
    profPullZ[dets[i]]     = new TProfile(Form("profNormRes_%s_VsZ",dets[i].Data()),
					  ...
					  10,-theZMax[i],theZMax[i],0.7,1.2,"i");
    ...
    h2MedianPhi[dets[i]]   = new TH2F(Form("h2_%s_VsPhi",dets[i].Data()),
				      ...
				      36,-TMath::Pi(),TMath::Pi(),nbins[dets[i]],mymap[dets[i]].first,mymap[dets[i]].second);
    ...
    h2MedianZ[dets[i]]     = new TH2F(Form("h2_%s_VsZ",dets[i].Data()),
				      ...
				      10,-theZMax[i],theZMax[i],nbins[dets[i]],mymap[dets[i]].first,mymap[dets[i]].second);
    ...
    h2PullPhi[dets[i]]     = new TH2F(Form("h2NormRes_%s_VsPhi",dets[i].Data()),
				      Form("Normalized Residuals RMS vs #varphi (%s);detector azimuth #varphi;r.m.s. of pull",dets[i].Data()),
				      36,-TMath::Pi(),TMath::Pi(),100,0.7,1.2);  
    
    h2PullZ[dets[i]]       = new TH2F(Form("h2NormRes_%s_VsZ",dets[i].Data()),
				      Form("Normalized Residuals RMS vs z (%s);detector z [cm];r.m.s. of pull",dets[i].Data()),
				      10,-theZMax[i],theZMax[i],100,0.7,1.2);  
    ...
    h1MedianPhi[dets[i]]   = new TH1F(Form("h1_%sRMSPhi",dets[i].Data()),
				       Form("RMS of DMR vs #varphi (%s);detector azimuth #varphi;r.m.s. of DMR [#mum]",dets[i].Data()),
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if (ientry < 0) break;
      ...
      // if(medianX*cmToUm>10){ 
      // 	std::cout<<medianX*cmToUm<<" "
      // 		 <<subDetId<<" "
      // 		 <<entries<<" "
      // 		 <<moduleId<<" "
      // 		 <<layer<<" "
	...
	// if(TMath::Abs(posPhi-2.)<0.1) {
	//   std::cout<<"medianx: "
	// 	   << medianX*cmToUm <<" subdetId: "
	// 	   << subDetId       <<" entries: "
	// 	   << entries        <<" moduleId: "
	// 	   << moduleId       <<" layer: "
	// 	   << layer          <<" posZ: "
