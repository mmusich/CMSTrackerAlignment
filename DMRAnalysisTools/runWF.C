...
#include "generalizedSuperImpose.C"

TString prepareString(const char *dirname=".", const char *ext=".root",TList *LabelList=0);
 ...
 // **********************************************
  ...
  for(Int_t j=0; j < nOfFiles; j++) {   
    ...
    if (l) {
      ...
      processline.Clear();
      processline = Form(".! mkdir plots_%s",append);
      gROOT->ProcessLine(processline.Data());
      processline.Clear();
    } else {
  ...
  if (m) {
    ...
    gROOT->ProcessLine(processline.Data());
    processline.Clear();
    processline = Form(".! mkdir %s",outCompDir.Data());
    gROOT->ProcessLine(processline.Data());
    processline.Clear();
    } else {
  ...
  gROOT->ProcessLine((".! mv *.png ./"+outCompDir).Data());
