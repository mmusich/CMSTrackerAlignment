...
command="./das_client.py --query=\"run dataset=/StreamExpress0T/Run2015D-TkAlMinBias-Express-v4/ALCARECO | grep run.run_number | sort run.run_number\" --limit=0 | awk \'$1>"+lastRun+"\'> RunNumbers.txt"
print command
os.system(command)
...
if len(RunLines) > 0:
    for run_num in RunLines:
        ...
        if isFinished == "True" or skipExpressStreamFinishedCheck:
            ...
            callCommand = "./das_client.py --query=\"file run="+run_num[:-1]+" dataset=/StreamExpress0T/Run2015D-TkAlMinBias-Express-v4/ALCARECO | grep file.name\" --limit=0 > tempFiles.txt"
            print callCommand
            ...
            if len(ListFiles) > 0:
                command = "./das_client.py --query=\"summary run="+run_num[:-1]+" dataset=/StreamExpress0T/Run2015D-TkAlMinBias-Express-v4/ALCARECO | grep summary.nevents\" --limit=0"
...
if float(bfield) > 0.25:
    ...
    command = "echo \"WARNING 0T Alignment used for Run " + runNumber + ", where bfield = "+str(bfield)+"\" | mail -s \"WARNING 0T Alignment Used By Mistake\" jcastle@cern.ch"
    os.system(command)    
